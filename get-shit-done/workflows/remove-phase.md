...
```bash
INIT=$(node /Users/isaiahrivera/.claude/get-shit-done/bin/gsd-tools.js init phase-op "${target}")
```
...
```bash
RESULT=$(node /Users/isaiahrivera/.claude/get-shit-done/bin/gsd-tools.js phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node /Users/isaiahrivera/.claude/get-shit-done/bin/gsd-tools.js phase remove "${target}" --force)
```
...
```bash
node /Users/isaiahrivera/.claude/get-shit-done/bin/gsd-tools.js commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
