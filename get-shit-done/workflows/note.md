...
- **Global scope**: `/Users/isaiahrivera/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/isaiahrivera/.claude/notes/`)
...
2. Glob `/Users/isaiahrivera/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/isaiahrivera/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/isaiahrivera/.claude/notes/` — works in any directory
