...
const runCurrent = (cwd, extraEnv = {}) =>
	JSON.parse(
		execFileSync("node", ["scripts/project-context.mjs", "current"], {
			cwd: root,
			encoding: "utf8",
			env: {
				...process.env,
...
test("same repo path yields the same project id", () => {
	const tmpDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "agents-project-context-"),
	);
...
test("bootstrapping project context resolves correct path shape", () => {
	const tmpDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "agents-project-context-"),
	);
	...
	try {
		...
		assert.equal(
			context.thoughtPaths.plans,
			path.join(repoRoot, "thoughts", "plans"),
		);
		assert.equal(
			context.thoughtPaths.research,
			path.join(repoRoot, "thoughts", "research"),
		);
		assert.equal(
			context.thoughtPaths.sessions,
			path.join(repoRoot, ".omx", "sessions"),
		);
		assert.equal(
			context.thoughtPaths.handoffs,
