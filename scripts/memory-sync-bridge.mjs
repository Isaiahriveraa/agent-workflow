...
export const buildMemorySyncStatus = ({ env = process.env, projectContext = ensureProjectContext() } = {}) => {
  ...
  return {
    ...
    continuity: {
      statePath: projectContext.contextPaths.state,
      researchIndexPath: projectContext.contextPaths.researchIndex,
      sessionIndexPath: projectContext.contextPaths.sessionIndex,
      lessonQueueDir: path.join(projectContext.thoughtPaths.lessons, 'queue', projectContext.projectSlug)
    },
    recommendedCommands: {
      preferred: [
...
const usage = (scriptName = 'memory-sync-bridge.mjs') => {
  console.error(`Usage: node scripts/${scriptName} <status|recall|flush> [--workflow-stage <create-plan|implement-plan>] [--query text] [--context-summary text]`);
  process.exitCode = 1;
};
...
export const runMemorySyncBridge = async ({
  ...
  switch (command) {
    case 'status':
      console.log(JSON.stringify(buildMemorySyncStatus({ env, projectContext }), null, 2));
      break;
    case 'recall': {
