...
const detectAdapterStatus = () => {
	const manifest =
		readJson(path.join(project.agentsRoot, "manifest.json")) ?? {};
	const capabilities = manifest.capabilities ?? {};
	const summarize = (tool, fields) => {
		const entry = capabilities[tool] ?? {};
		return Object.fromEntries(
			fields.map((field) => [field, entry[field]?.status ?? "unknown"]),
		);
	};

	return {
		"claude-code": summarize("claude-code", [
			"hooks",
			"session_continuity",
			"settings",
		]),
		opencode: summarize("opencode", [
			"commands",
			"agents",
			"hooks",
			"session_continuity",
		]),
		"codex-cli": summarize("codex-cli", [
			"agents",
			"hooks",
			"session_continuity",
		]),
		antigravity: summarize("antigravity", [
			"commands",
			"agents",
			"hooks",
			"session_continuity",
		]),
		openclaw: summarize("openclaw", [
			"workspace_wrappers",
			"memory",
			"session_continuity",
		]),
...
const detectVerification = () => {
	const packageJson =
		readJson(path.join(project.projectRoot, "package.json")) ?? {};
	const scripts = packageJson.scripts ?? {};
	return {
		available:
			typeof scripts.test === "string" ||
			typeof scripts["validate:ssot"] === "string",
		preferred: ["validate:ssot", "test"].filter(
...
const detectContinuity = () => {
	let stateContent = "";
	try {
		stateContent = fs.readFileSync(project.contextPaths.state, "utf8");
	} catch {}
	const workingSetSection = readSection(
		stateContent,
...
const buildActions = ({ continuity, execution, router, memory }) => {
	...
	if (execution.present && execution.stale) {
		actions.push({
			...
			severity: "high",
			rationale:
				"Execution state is still marked active or paused, but its last update is older than the stale threshold.",
			recommended_command: "/stop-work",
			suggested_transition:
				execution.status === "paused" ? "resume or clear" : "pause or stop",
		});
	}
	...
	if (
		...
		actions.push({
			...
			severity: "medium",
			rationale:
				"Execution state is terminal and should be cleared before more continuation logic runs.",
			recommended_command: "/stop-work",
	...
	if (!execution.present && continuity.related_plan !== "none") {
		actions.push({
			...
			severity: "medium",
			rationale:
				"Workflow state points at an active plan, but there is no execution sidecar to track current task progress.",
	...
	if (
		!execution.present &&
		continuity.session_selected &&
		continuity.session_path !== "none"
	) {
		actions.push({
			issue: "stale_session_selection",
			severity: "medium",
			rationale:
				"The working set still points at a session artifact even though no active execution state exists.",
	...
	if (
		!execution.present &&
		continuity.handoff_selected &&
		continuity.handoff_path !== "none"
	) {
		actions.push({
			issue: "stale_handoff_selection",
			severity: "medium",
			rationale:
				"The working set points at a handoff artifact and no active execution state is present.",
...
export const buildDoctorReport = () => {
	...
	if (execution.stale) {
		warnings.push(
			"execution state appears stale and should be paused, stopped, resumed, or cleared deliberately",
		);
	}
	if (
		execution.present &&
		...
		warnings.push(
			"execution state is terminal and should be cleared before more continuation work starts",
		);
	}
	if (
		!execution.present &&
		...
		warnings.push(
			"working set still points at a session artifact without active execution state",
		);
	}
	if (
		!execution.present &&
		...
		warnings.push(
			"working set points at a handoff artifact without active execution state",
		);
	}
	if (!router.config_valid) {
