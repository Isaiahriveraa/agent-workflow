...
const adapterReadmeHeadings = new Map([
	[
		"adapters/claude-code/README.md",
		["# Claude Code Adapter", "## Capability Profile", "## Canonical Boundary"],
	],
	[
		"adapters/codex-cli/README.md",
		[
...
const commandContracts = new Map([
	[
		...
		[
			...
			"scripts/workflow-command-decision.mjs evaluate",
			"do_more_research",
			"run_critic",
			...
			"Phase Plan Index",
			"one child plan per implementation phase",
	...
	[
		...
		[
			...
			"advisory memory recall",
			"disabled mode must preserve current behavior",
			"Do not pass advisory recall into `scripts/workflow-artifact-tools.mjs`",
			"scripts/workflow-command-decision.mjs evaluate",
			"replan",
			"capture_lesson",
			"request_user_decision",
			"Never retry blindly after a verified miss",
			"scripts/lesson-tools.mjs quick-capture",
			"Phase Plan Index",
			"one linked child phase plan per explicit implementation phase",
			"read the linked child phase plan for that phase before editing",
		],
	],
	[
	...
	[
		"commands/project-tooling.md",
		["~/.agents/contexts/tooling.md", "scripts/package-manager-tools.mjs"],
	],
	[
		"commands/project-verification.md",
		["~/.agents/contexts/verification.md", "scripts/verification-tools.mjs"],
	],
	["commands/project-artifacts.md", ["scripts/artifact-tools.mjs"]],
]);
...
const explicitOutputContracts = new Map([
	[
		"commands/rpi-brainstorm.md",
		["Next step", "/research_codebase /absolute/path/to/intake.md"],
	],
	[
		"commands/research_codebase.md",
	...
	[
		"commands/implement_plan.md",
		["Next step", "/validate_plan /absolute/path/to/plan.md"],
	],
	[
		"commands/validate_plan.md",
		[
			"Do not emit a standalone `Next step` command block from this command unless the user explicitly asks for a specific follow-up command.",
		],
	],
]);
...
for (const [relativePath, references] of commandContracts) {
	...
	for (const reference of references) {
		if (!content.includes(reference)) {
			console.error(
				`Missing command contract reference "${reference}" in ${relativePath}`,
			);
			hasError = true;
		}
	}
...
for (const [relativePath, references] of handoffContracts) {
	...
	for (const reference of references) {
		if (!content.includes(reference)) {
			console.error(
				`Missing handoff contract reference "${reference}" in ${relativePath}`,
			);
			hasError = true;
		}
	}
...
for (const [relativePath, references] of explicitOutputContracts) {
	...
	for (const reference of references) {
		if (!content.includes(reference)) {
			console.error(
				`Missing explicit output contract "${reference}" in ${relativePath}`,
			);
			hasError = true;
		}
	}
...
for (const [relativePath, references] of continuityContracts) {
	...
	for (const reference of references) {
		if (!content.includes(reference)) {
			console.error(
				`Missing continuity contract "${reference}" in ${relativePath}`,
			);
			hasError = true;
		}
	}
...
if (fs.existsSync(manifestPath)) {
	...
	const claudePromptLink = manifest.symlinks?.find(
		(entry) =>
	...
	if (!claudePromptLink) {
		...
		if (
			claudePromptLink.target !== "~/.agents/adapters/claude-code/CLAUDE.md"
		) {
			console.error(
				`Claude Code CLAUDE.md must target ~/.agents/adapters/claude-code/CLAUDE.md, found ${claudePromptLink.target}`,
			);
			hasError = true;
		}
		...
		if (!fs.existsSync(claudeEntrypointPath)) {
			console.error(
				`Claude Code entrypoint is missing: ${claudeEntrypointPath}`,
			);
			hasError = true;
		} else {
			...
			if (!stats.isSymbolicLink()) {
				console.error(
					`Claude Code entrypoint must be a symlink: ${claudeEntrypointPath}`,
				);
				hasError = true;
			} else {
	...
	const validatedClaudeSettings = manifest.validated_local_contracts?.find(
		(entry) =>
	...
	if (
		claudeCapabilities?.entrypoint?.contract !==
		"~/.claude/CLAUDE.md -> ~/.agents/adapters/claude-code/CLAUDE.md"
	) {
		console.error(
			"Manifest Claude Code capability contract does not match the wrapper entrypoint",
		);
		hasError = true;
	}
	...
	if (!fs.existsSync(settingsPath)) {
		...
		try {
			...
			const additionalDirectories = Array.isArray(
				...
				? settings.additionalDirectories
				: [];
			const permissionsAllow = Array.isArray(settings.permissions?.allow)
				? settings.permissions.allow
				: [];
			const claudeMdFlag =
				settings.env?.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD;

			if (!additionalDirectories.includes(path.join(home, ".agents"))) {
				console.error(
					`Claude settings missing additionalDirectories entry for ${path.join(home, ".agents")}`,
				);
				hasError = true;
			}

			if (
				!permissionsAllow.includes(`Read(${path.join(home, ".agents")}/**)`)
			) {
				console.error(
					`Claude settings missing Read(${path.join(home, ".agents")}/**) permission`,
				);
				hasError = true;
			}
			...
			if (claudeMdFlag !== "1") {
				console.error(
					'Claude settings must set env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD to "1"',
				);
				hasError = true;
			}

			const stopHooks = Array.isArray(settings.hooks?.Stop)
				? settings.hooks.Stop
				: [];
			const subagentStopHooks = Array.isArray(settings.hooks?.SubagentStop)
			...
			const hasStopHook = stopHooks.some(
				(matcher) =>
					...
					matcher.hooks.some(
						(hook) =>
							hook.command ===
							`node "${path.join(home, ".claude", "hooks", "gsd-stop-memory-consolidation.cjs")}"`,
					),
			);
			const hasSubagentStopHook = subagentStopHooks.some(
				(matcher) =>
					Array.isArray(matcher.hooks) &&
					matcher.hooks.some(
						(hook) =>
			...
			if (!hasStopHook) {
				console.error(
					"Claude settings missing Stop hook for gsd-stop-memory-consolidation.cjs",
				);
				hasError = true;
			}
			...
			if (!hasSubagentStopHook) {
				console.error(
					"Claude settings missing SubagentStop hook for gsd-stop-memory-consolidation.cjs",
				);
				hasError = true;
			}
		} catch (error) {
...
if (
	agentsContent.includes(
		"Use `thoughts/sessions/` for ordinary workflow continuity",
	)
) {
	console.error(
		"AGENTS.md still references legacy thoughts/sessions continuity for ordinary sessions",
	);
	hasError = true;
}
