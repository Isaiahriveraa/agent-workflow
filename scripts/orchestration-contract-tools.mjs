...
const readStateContext = () => {
	let content = "";
	try {
		content = fs.readFileSync(project.contextPaths.state, "utf8");
	} catch {}
	return {
...
const deriveAccelerationMode = (provider) => {
	if (provider === "claude-code") {
		return {
			...
			rationale:
				"Claude Code is the first adapter in this repo with native hook-backed workflow acceleration.",
		};
	}
	...
	return {
		...
		rationale:
			"This adapter does not currently have native orchestration enforcement in the repo, so the contract is conventional rather than runtime-enforced.",
	};
};

const buildConductorResponsibilities = ({
	state,
	execution,
	executionGuidance,
}) => {
	...
	if (execution?.state?.active_plan) {
		responsibilities.push(
			`Stay anchored to the active execution plan: ${execution.state.active_plan}.`,
		);
	} else if (state.relatedPlan !== "none") {
		responsibilities.push(
			`Use the tracked plan as the orchestration authority: ${state.relatedPlan}.`,
		);
	}

	if (executionGuidance.current_task?.label) {
		responsibilities.push(
			`Current task focus: ${executionGuidance.current_task.label}.`,
		);
	}
...
export const buildOrchestrationContract = ({
	...
	if (!executionGuidance.present) {
		warnings.push(
			"No active execution state is present; initialize or resume execution before orchestration delegates work.",
		);
	}
	...
	return {
		...
		intent_reset: {
			...
			rationale:
				"Reset turn-local intent before orchestration so conductor mode does not drift across turns.",
		},
		execution: execution
			? {
					path: execution.path,
		...
		conductor: {
			...
			responsibilities: buildConductorResponsibilities({
				state,
				execution,
				executionGuidance,
			}),
		...
		executor: {
			...
			learnings_handoff: {
				notes_dir: notesDir,
				required_files: notesDir
					? NOTE_FILES.map((file) => path.join(notesDir, file))
					: [],
		...
		recommended_next_command:
			executionGuidance.cleanup?.recommended_command ??
	...
	return {
		...
		delegated_task: {
			task_label:
				...
				contract.execution_guidance.current_task?.label ??
				"current task",
...
if (
	...
	if (command === "contract") {
		...
		console.error(
			'Usage: node scripts/orchestration-contract-tools.mjs <contract|decide> [--input "..."] [--provider ...] [--intent ...] [--category ...]',
		);
		process.exitCode = 1;
	}
}
