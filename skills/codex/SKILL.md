...
## Running a Task
1. Ask the user (via `AskUserQuestion`) which model to run (`gpt-5.4`, `gpt-5.4-mini`, or `o3`) AND which reasoning effort to use (`xhigh`, `high`, `medium`, or `low`) in a **single prompt with two questions**.
