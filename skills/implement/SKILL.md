...
You are tasked with implementing an approved technical plan from `plan server `. These plans contain phases with specific changes and success criteria.
...
**Next step:** `/skill:validate plan server {filename}.md` — verify the implementation against the plan's success criteria before committing.
