...
1. **Determine input mode**:
   ...
   **Design artifact provided** (path to a `.md` file in `plan server `):
   ...
   **No arguments provided**:
   ...
   `/skill:plan plan server 2025-01-20_09-30-00_feature.md`
...
1. **Write the plan skeleton** to `plan server <slug>_<description>.md` (use `<slug>` from the Metadata block's `now.mjs` line 1; copy `<iso>` verbatim into frontmatter `date:` and `last_updated:`).
...
- Design: `plan server {file}.md`
- Research: `plan server {file}.md`
...
> 💡 **Browser viewing**: To view this plan in the plan server, run: `/skill:plan-server --title "Plan Title" --project <project>`. The plan server serves plans at `http://localhost:3456/project/{project}/plans`.
   Please review:
...
After writing the plan artifact to disk, immediately run `bash` with `pbcopy <absolute-path>` (using the Write tool's resolved file path) so the user's clipboard has the file path.
