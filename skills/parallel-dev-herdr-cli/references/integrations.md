...
### Binary distinction for this fork

The runtime-control commands in this reference (`herdr workspace`, `herdr pane`, and `herdr wait`) remain governed by the official `herdr` skill and the Herdr daemon's runtime CLI. Use the fork binary (`herdr-dev`, or the exact `$PWD/target/debug/herdr` / `$PWD/target/release/herdr`) for inbox-facing commands and for launching workers when their child commands must report through the fork. In other words: use the official runtime integration for workspace/pane lifecycle, and use the fork executable for `inbox create`, `enroll`, `send`, `read`, `wait`, `watch`, and `ask`.
