---
name: parallel-dev-herdr-cli
description: Execute validated parallel development plans through isolated Git worktrees and Herdr-managed worker panes using the herdr-dev CLI inbox for structured coordinator-worker reports, correlated asks, supervision, verification, and draft PRs. Use when parallel development must use this Herdr fork and its inbox workflow.
---
...
Commander:   plan validation, approval gate, runtime delegation,
             inbox setup and report consumption, STATUS supervision,
             commander verification, human-merge detection
Worker pane: boss protocol (spawn → subagent-implementation-review → commit
             → pr-workflow), STATUS messages, inbox milestone/blocker/completion
             reports, draft PR creation
Human:       issue approval, PR review, and merge authority
Herdr:       pane and OMP runtime control; herdr-dev inbox coordination
```
...
For this fork, the commander creates one named `herdr-dev inbox` per coordination group. The inbox supplements the canonical `STATUS` transport: STATUS remains the phase-control signal, while inbox reports carry concise structured milestones, blockers, completion evidence, and correlated worker answers.
...
For this fork, use the official Herdr runtime commands for workspace and pane lifecycle, but use the fork executable for inbox operations. Establish a distinct session and binary before launching workers:

```bash
export HERDR_DEV_BIN="${HERDR_DEV_BIN:-$PWD/target/debug/herdr}"
export PATH="$(dirname "$HERDR_DEV_BIN"):$PATH"
herdr-dev --session "<session-name>"
```

The session name is part of the runtime isolation contract. Do not attach a worker to an unrelated default session.

### Coordinator inbox setup

Before the first worker launch in a coordination group, create the named inbox from the coordinator context:

```bash
herdr-dev inbox create "<inbox-name>"
```

Enroll each recognized worker after its pane exists:

```bash
herdr-dev inbox enroll "<inbox-name>" "<agent-target>"
```

Enrollment sends the worker the reporting contract and the exact fork executable path. Record the inbox name in the worker's context packet and `.herdr/state.yaml` so every worker reports to the same coordinator inbox.
...
The context packet must tell the worker to report meaningful milestones, blockers, and completion to the assigned inbox, for example:

```bash
herdr-dev inbox send "<inbox-name>" "IMPLEMENTING: started issue <issue-key>"
herdr-dev inbox send "<inbox-name>" "BLOCKED: <actionable reason>"
herdr-dev inbox send "<inbox-name>" "DONE: <sha>; <verification result>"
```

These reports supplement, rather than replace, the worker's required `STATUS` messages.
...
### Inbox report handling

The inbox is the structured coordinator-worker handoff channel for this fork. After receiving a STATUS transition or when supervising a worker, the commander reads the named inbox rather than scraping pane output:

```bash
herdr-dev inbox read "<inbox-name>" --after "$LAST_SEQUENCE" --json
herdr-dev inbox wait "<inbox-name>" --after "$LAST_SEQUENCE" --timeout 30000 --json
```

Use `inbox watch` for an interactive coordinator view and `inbox ask` for a targeted follow-up:

```bash
herdr-dev inbox watch "<inbox-name>" --after "$LAST_SEQUENCE"
herdr-dev inbox ask "<inbox-name>" "<agent-target>" "<question>" --timeout 30000
```

Advance the cursor from returned sequences, including when a page is filtered or reports `truncated_before: true`. Do not resend a generic nudge merely because no report has arrived; inspect the authoritative Herdr runtime state and the worker's Git evidence first. A blocker report warrants one precise response, while a completion report still requires exact SHA, diff, and verification checks.
...
## Herdr CLI fork context

This skill specializes parallel development for the personal Herdr fork that includes coordinator-worker inboxes.

### Binary and worktree selection

Use the fork explicitly. The installed command is `~/.local/bin/herdr-dev` (currently Herdr 0.7.5 with the inbox feature). During development, prefer the exact binary for the current worktree: `$PWD/target/debug/herdr` or `$PWD/target/release/herdr`. Never rely on an unqualified `herdr` from `PATH` when launching workers; it may be the older installation. The `inbox enroll` implementation uses the current executable path so prompted workers invoke the same fork automatically.

The repository is `/Users/isaiahrivera/Documents/Github/herdr`; the implementation branch is `feat/pane-inbox`. Workers own implementation worktrees and issue branches; the commander owns Herdr workspace/pane lifecycle and must not edit feature code. Prepend the debug directory to PATH before launching child panes:

```bash
export PATH="$PWD/target/debug:$PATH"
```

Use a distinct named session for each smoke test or development run, for example `herdr-dev --session inbox-dev`. Inbox state is bounded and session-memory-only; a Herdr server restart clears inboxes. Keep the existing `herdr` installation untouched unless the user explicitly requests replacement.

### Coordinator-worker inbox protocol

The inbox is the structured reporting channel; do not scrape terminal output to infer worker progress. A coordinator creates a named inbox and enrolls recognized worker agents:

```bash
herdr-dev inbox create <name> [--pane <pane_id>]
herdr-dev inbox enroll <name> <agent-target>
```

Workers send concise milestone, blocker, and completion reports:

```bash
herdr-dev inbox send <name> "<concise status>"
```

The coordinator consumes reports with:

- `herdr-dev inbox read <name> --after <sequence>` for a one-shot ordered page.
- `herdr-dev inbox wait <name> --after <sequence> --timeout <milliseconds>` for a one-shot wait.
- `herdr-dev inbox watch <name> --after <sequence>` for live reports and replay after reconnect.
- `herdr-dev inbox list` to inspect inboxes.
- `herdr-dev inbox ask <name> <agent-target> "<question>" --timeout <milliseconds>` for a safe coordinator-to-worker question through the existing agent prompt path and reply token.

Use `--json` for machine processing. Human report lines are tab-separated as `sequence<TAB>source<TAB>message`. Reports are bounded to 256 per inbox; handle `truncated_before: true` as a signal that older reports were evicted and advance the cursor from the returned page rather than retrying stale history. Preserve source-pane and reply-token filters for correlated asks.

### Worker reporting contract

An enrolled worker reports only meaningful milestones, blockers, and completion—not every command or token. Examples:

```bash
herdr-dev inbox send main "IMPLEMENTING: schema wiring complete; running focused tests"
herdr-dev inbox send main "BLOCKED: Windows quoting test exposes an API contract mismatch"
herdr-dev inbox send main "DONE: committed <sha>; focused tests passed"
```

Workers still emit the canonical single-line `STATUS` messages required by this skill. The inbox supplements STATUS with structured in-session reports; it does not replace commit, branch, diff, verification, or evidence requirements.

### Herdr-specific supervision guidance

- Before sending a pane message or asking a worker, inspect authoritative runtime state with the fork's pane commands and confirm the target still owns the foreground agent.
- Treat `idle`, `working`, `blocked`, `done`, and `unknown` as runtime states, not task completion claims. Verify inbox reports against Git evidence.
- Prefer `inbox wait` or `inbox watch` over polling pane output for worker milestones.
- If a worker is blocked, read the latest inbox report, inspect the relevant worktree/diff, and send one precise follow-up. Do not send repeated generic nudges while it is `working`.
- When a worker completes, verify the exact commit SHA, branch, worktree, tests, and scope before sending the normal `VERIFICATION_PASSED` handshake.

For the command matrix and smoke-test sequence, read `references/herdr-cli.md` in this skill directory. The copied `references/` files retain the general parallel-dev contracts and workflow.
