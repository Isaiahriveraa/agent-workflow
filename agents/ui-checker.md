...
Runs after the UI researcher creates UI-SPEC.md for verification or re-verification.
...
> Skip this dimension entirely if `workflow.ui_safety_gate` is explicitly set to `false` in `"$CONTEXT_DIR"/config.json`. If the key is absent, treat as enabled.
...
fix_hint: "Re-run the UI phase to trigger the registry vetting gate, or manually run 'npx shadcn view {block} --registry {url}' and record results"
```
```yaml
dimension: 6
severity: PASS
...
### Action Required
Fix blocking issues in UI-SPEC.md and re-run the UI phase.
```
